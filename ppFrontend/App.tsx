...
import {
  ...
  OptimizeSSEEvent, TokenUsageBreakdown, ExperimentDetail
} from './types';
...
type ViewMode = 'optimizer' | 'history-list' | 'history-detail';
...
const App: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<ViewMode>('optimizer');
  ...
  const loadExperimentIntoOptimizer = (exp: ExperimentDetail) => {
    const runnerModel = exp.runnerModel as any;
    setState(prev => ({
      ...prev,
      taskDescription: exp.taskDescription,
      basePrompt: exp.basePrompt,
      dataset: exp.datasetRows.map(dr => ({
        id: dr.id,
        query: dr.query,
        expectedOutput: dr.expectedOutput,
        softNegatives: dr.softNegatives || undefined,
        hardNegatives: dr.hardNegatives || undefined,
      })),
      runnerModel: {
        provider: (runnerModel?.provider as LLMProvider) || LLMProvider.GEMINI,
        model: runnerModel?.model || 'gemini-3-flash-preview',
        settings: runnerModel?.settings || { temperature: 0.1 },
      },
      juryMembers: exp.juryMembers.map(jm => ({
        id: jm.id,
        name: jm.name,
        provider: jm.provider as LLMProvider,
        model: jm.model,
        settings: { temperature: 0, ...(jm.settings as Partial<ModelSettings>) },
      })),
    }));
      ...
      <main className="flex-1 overflow-y-auto p-6 lg:p-12 relative bg-[#05070a]">
        <div className="max-w-5xl mx-auto space-y-12 pb-32">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => { setViewMode('history-list'); setSelectedExperimentId(null); }}
              className={`px-5 py-2.5 rounded-2xl text-xs font-black uppercase tracking-wider transition-all ${
                viewMode === 'history-list' || viewMode === 'history-detail'
                  ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/40'
                  : 'bg-white/[0.03] text-gray-500 hover:text-gray-300 border border-white/5'
              }`}
            >
          ...
          {viewMode === 'history-list' && (
            <ExperimentList onSelectExperiment={(id) => { setSelectedExperimentId(id); setViewMode('history-detail'); }} />
          )}
          {viewMode === 'history-detail' && selectedExperimentId && (
            <ExperimentDetailView
              experimentId={selectedExperimentId}
              onBack={() => setViewMode('history-list')}
              onLoadIntoOptimizer={loadExperimentIntoOptimizer}
            />
          )}
